...
## 2024-05-24 - HTTP Header Injection via Unsanitized Filename
**Vulnerability:** Several controllers (`client.controller.js`, `case.controller.js`, `docketFileStorage.controller.js`) interpolated untrusted user-supplied filenames directly into the `Content-Disposition` HTTP header during file downloads, creating an HTTP Header Injection risk.
**Learning:** Developers frequently set attachment names dynamically without considering that carriage returns or line feeds in the filename could split the header and allow injecting arbitrary HTTP headers.
**Prevention:** To prevent HTTP Header Injection and related vulnerabilities during file downloads, always pass untrusted filenames through `sanitizeFilename` from `src/utils/fileUtils` before interpolating them into a `Content-Disposition` header.
