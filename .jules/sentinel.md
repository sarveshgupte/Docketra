...
## 2026-07-20 - Prevent Hardcoded Secret Logging
**Vulnerability:** A hardcoded, plaintext `SYSTEM_HASH_SECRET` was committed in `docs/operations/cloudmailin_inbound_integration.md`.
**Learning:** Hardcoded cryptographic secrets in documentation or configuration files expose systems to unauthorized access and fail SAST secret scanning checks.
**Prevention:** Always use allowed placeholder formats (like `<required-base64-secret>` matching the `.gitleaks.toml` allowlist) when documenting required environment variables.
