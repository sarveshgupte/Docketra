...
## 2026-08-16 - Mass Assignment / IDOR via `...req.body`
**Vulnerability:** Spread operator `...req.body` used to construct database update or create payloads.
**Learning:** In `src/controllers/complianceTemplate.controller.js` and `src/controllers/form.controller.js`, `req.body` was being directly spread into database payloads. This allows attackers to mass-assign protected fields (like `_id`, `firmId`, `createdByXID`) that were not explicitly overwritten in the remainder of the object block if they inject them into the request body.
**Prevention:** When using `...req.body` to construct MongoDB update or create payloads, explicitly `delete` protected root-level fields (e.g., `_id`, `firmId`, `createdByXID`, `updatedByXID`) from the cloned request body before assigning trusted server-side context, to prevent mass assignment and IDOR vulnerabilities.
