...
## 2024-07-08 - [Mass Assignment / IDOR Vulnerability in Compliance Template Controller]
**Vulnerability:** The `createComplianceTemplate` and `updateComplianceTemplate` controllers spread `...req.body` without filtering, which allowed malicious actors to overwrite sensitive internal properties like `_id`. Although `firmId`, `createdByXID`, and `updatedByXID` were overridden directly in the object literal assignment which gave them precedence, if they appeared nested in `$set`, or if an attacker provided an unexpected object schema for `_id`, it could still lead to issues or crash the query.
**Learning:** Object spread syntax without a whitelist or safe filter allows mass assignment and potential modification of core document properties.
**Prevention:** Always create a cloned `safeBody` and explicitly `delete` protected root-level fields (e.g., `_id`, `firmId`, `createdByXID`, `updatedByXID`) before using the body in an update payload.
