...
## 2026-08-01 - Predictable ID Generation and Stack Trace Leakage in Error Responses\n**Vulnerability:** Used Math.random() for reqId generation and exposed error.stack in internal server error responses.\n**Learning:** Math.random() produces predictable values which can lead to token collisions and security vulnerabilities, while exposing stack traces leaks sensitive internal implementation details to end users and logs when debug mode is accidentally enabled.\n**Prevention:** Always use the built-in Node.js crypto module (e.g., crypto.randomUUID()) for generating identifiers and correlation IDs. Ensure error responses never include error.stack; log detailed errors server-side instead.
