...
## 2024-05-28 - Replace Math.random() with crypto.randomUUID()
...
**Learning:** `Math.random()` is cryptographically insecure and predictable, which could allow an attacker to guess generated request IDs and gain unauthorized access to request details.
**Prevention:** Always use Node.js's native `crypto` module (`crypto.randomUUID()`) for generating unique identifiers.
