...
## 2026-09-05 - Prevent Information Exposure via Error Messages in DocumentItem Controller
**Vulnerability:** Raw error messages (`error.message`) were being directly exposed to clients in API error responses in `src/controllers/documentItem.controller.js`.
**Learning:** Exposing raw internal error details to the client can leak sensitive system information, configuration details, or underlying infrastructure state, leading to Information Exposure (CWE-200).
**Prevention:** Always log the full error details server-side using the internal logger (`log.error`) and return generic, safe error messages to the client for 500 status codes.
