...
## 2024-05-22 - Mass Assignment and IDOR Vulnerability in Compliance Templates
**Vulnerability:** The application was trusting client-provided audit fields (`createdByXID`, `updatedByXID`) and structural fields (`firmId`) from `req.body` directly via `...req.body` spreading in the `$set` object during `ComplianceObligationTemplate.findOneAndUpdate`. This allowed attackers to manipulate audit records or reassign templates to other tenants.
**Learning:** This pattern exposes the application to mass assignment and IDOR vulnerabilities. While creating a template explicitly overrode `firmId` in the payload, the update function only provided a partial override (`updatedByXID`), leaving other sensitive properties exposed to overwrites.
**Prevention:** To prevent Mass Assignment and IDOR vulnerabilities, never blindly spread `req.body` into database update objects. Always create a shallow clone of the input and explicitly `delete` protected fields (e.g., `_id`, `firmId`, `createdByXID`, `updatedByXID`) before merging them with server-derived authenticated context like `req.user`.
