...
## 2024-07-11 - [ARIA state for Sidebar Navigation Toggle]
**Learning:** Found the main layout sidebar toggle buttons lacking `aria-controls` bindings to pair with `aria-expanded`. Without `aria-controls` bindings, screen reader users wouldn't know exactly which element changes state when toggled.
**Action:** Always ensure disclosure buttons/dropdown triggers have an explicit `aria-controls="[target-id]"` attribute connected to the `id` of their respective content panels, and add the matching `id` to the target content container.
