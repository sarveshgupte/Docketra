...
## 2024-05-30 - [ARIA controls for Layout Dropdowns]
**Learning:** The notification and user profile dropdown toggles in the application header used `aria-expanded` and `aria-haspopup` but lacked the critical `aria-controls` attribute linking them to their corresponding dropdown menus.
**Action:** Always ensure disclosure buttons/dropdown triggers have an explicit `aria-controls="[dropdown-id]"` attribute connected to the `id` of their respective content panels when implementing custom drop-downs.
