...
assert.ok(createDocketFormSource.includes('Object.keys(nextErrors).length > 0'), 'Create docket final submit should validate requirements before API submit.');
assert.ok(createDocketFormSource.includes('!formData.categoryId) nextErrors.categoryId'), 'Create docket should enforce required category validation.');
...
assert.ok(createDocketFormSource.includes('isDefaultClient || item.isSystemClient'), 'Create docket flow should communicate default firm client behavior');
