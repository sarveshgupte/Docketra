...
assert.ok(clientsPage.includes('Verify with OTP to'), 'Clients page must require OTP-backed confirmation for activate/deactivate.');
...
assert.ok(clientsPage.includes('Firm-owned default client'), 'Clients page should label the firm-owned default client on the card.');
for (const requiredField of ['businessAddress', 'contactPersonName', 'contactPersonEmail', 'contactPersonPhone', 'contactPersonEmailAddress', 'contactPersonPhoneNumber', 'PAN', 'CIN', 'TAN', 'GST']) {
  assert.ok(clientsPage.includes(requiredField), `Clients page form/payload should include ${requiredField}.`);
}
for (const requiredSection of ['Basic Organization Details', 'Contact Person & Channels', 'Statutory & Registration Identification']) {
