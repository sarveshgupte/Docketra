...
const repoRoot = isUiCwd ? path.resolve(process.cwd(), '..') : process.cwd();
...
const timeoutPolicyDoc = fs.readFileSync(path.join(repoRoot, 'docs/features/pr-history/session-timeout-policy.md'), 'utf8');
...
assert.ok(authContextSource.includes('checkIdleAndScheduleNextRef.current();'), 'Cross-tab storage activity should reuse centralized idle scheduling helper.');
...
assert.ok(!apiSource.includes("wrapHistoryMethod('replaceState')"), 'replaceState should not be treated as activity to avoid background URL-sync extending sessions.');
assert.ok(timeoutPolicyDoc.includes('not background keepalive/refetch'), 'Session timeout policy docs must state that background refetch/keepalive does not extend idle time.');
