...
assert.ok(
  source.indexOf('const employeeContextEnabled = selectedSubcategory?.employeeContextEnabled === true;')
    < source.indexOf('useEffect(() => {\n    if (employeeContextEnabled) return;'),
  'employeeContextEnabled must be declared before useEffect dependency evaluation to avoid runtime TDZ/ReferenceError.',
);
