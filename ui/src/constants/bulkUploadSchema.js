...
export const BULK_UPLOAD_SCHEMA = {
  team: {
    ...
    fields: [
      ...
      { key: 'role', required: true, description: 'Admin, Manager, or Employee', aliases: ['user_role'], validator: (value) => ['admin', 'manager', 'user', 'employee', 'staff'].includes(String(value || '').trim().toLowerCase()), validatorMessage: 'must be admin, manager, or employee' },
  ...
  clients: {
    helperText: ['Required: business/contact/address fields. PAN/GST/TAN/CIN are optional.'],
    fields: [
      { key: 'businessName', required: true, description: 'client legal name', aliases: ['name', 'client_name'] },
      { key: 'businessEmail', required: true, description: 'optional', aliases: ['email', 'client_email'], validator: (value) => EMAIL_REGEX.test(String(value || '').trim()), validatorMessage: 'must be a valid email' },
      { key: 'primaryContactNumber', required: true, description: 'optional', aliases: ['phone', 'mobile'] },
      { key: 'businessAddress', required: true, description: 'optional', aliases: ['address', 'client_address'] },
