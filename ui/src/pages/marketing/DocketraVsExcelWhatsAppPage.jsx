...
const MIGRATION_CARDS = [
  {
    num: '01',
    title: 'Client Master Importer',
    description: 'Upload CSV or Excel spreadsheets with client entities. Auto-validates 21-digit CIN, PAN, GSTIN, and TAN formats with pre-flight dry runs.',
    badge: '✓ Schema validation + Error CSV',
  },
  {
    num: '02',
    title: 'Quick Bulk Paste',
    description: 'Copy tab-separated cells directly from Microsoft Excel or Google Sheets and paste them right into Docketra without exporting or downloading files.',
    badge: '✓ Direct clipboard copy-paste',
  },
  {
    num: '03',
    title: 'Historical Docket Importer',
    description: 'Download pre-formatted templates to bring multi-year filing histories, ongoing secretarial matters, and past annual compliance records into your firm brain.',
    badge: '✓ Multi-year history migration',
  },
  {
    num: '04',
    title: 'Team & Taxonomy Bulk Setup',
    description: 'One-click mapping for your partners, managers, and associates, plus customized compliance categories (ROC, GST, IT, FEMA, Trademark).',
    badge: '✓ Automated role provisioning',
  },
];
      ...
      <header className="border-b border-slate-800/80 bg-slate-950/80 backdrop-blur-sm px-4 py-4 sm:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              Join Pilot<span className="hidden sm:inline"> (3 Months Free)</span>
            </Link>
          </div>
        </div>
      </header>
            ...
            <div className="rounded-xl border border-red-900/30 bg-red-950/10 p-4 sm:p-5">
              ...
              <div className="space-y-2.5 text-xs text-slate-300">
                ...
                <div className="rounded-lg bg-slate-900/80 p-2.5 border border-red-900/20">
                  ...
                  <p className="italic text-slate-300">"Where is the DIN KYC OTP? Did we file AOC-4 yesterday? Who has DSC PIN?"</p>
                </div>
                <div className="rounded-lg bg-slate-900/80 p-2.5 border border-red-900/20 text-red-300 text-[11px]">
                  Result: Partner spends 3 hours chasing files. Filing delayed. ROC late fees applied.
                </div>
              </div>
            </div>
      ...
      <section className="border-t border-slate-800/90 bg-gradient-to-b from-slate-900/70 to-slate-950 py-16 sm:py-24 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {MIGRATION_CARDS.map((card, idx) => (
              <div
                key={idx}
                className="rounded-2xl border border-slate-800 bg-slate-950 p-6 flex flex-col justify-between hover:border-amber-500/50 transition-all"
              >
                <div>
        ...
        <div className="mx-auto max-w-5xl rounded-3xl border border-slate-800 bg-slate-950 p-6 sm:p-10 shadow-2xl">
          <div className="text-center max-w-2xl mx-auto mb-8">
            ...
            <div className="mt-6 flex flex-wrap sm:inline-flex justify-center rounded-xl bg-slate-900 p-1 border border-slate-800 text-xs gap-1 max-w-full">
              <button
                type="button"
                onClick={() => setActiveSandboxTab('memory')}
                className={`px-3 py-1.5 rounded-lg font-semibold transition-all ${
                  activeSandboxTab === 'memory' ? 'bg-amber-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-white'
                }`}
              >
                Client Memory
              </button>
              <button
                type="button"
                onClick={() => setActiveSandboxTab('qc')}
                className={`px-3 py-1.5 rounded-lg font-semibold transition-all ${
                  activeSandboxTab === 'qc' ? 'bg-amber-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-white'
                }`}
              >
                4-Eye QC Gate
              </button>
              <button
                type="button"
                onClick={() => setActiveSandboxTab('importer')}
                className={`px-3 py-1.5 rounded-lg font-semibold transition-all ${
                  activeSandboxTab === 'importer' ? 'bg-amber-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-white'
                }`}
              >
            ...
            <div className="flex items-center justify-between border-b border-slate-800 px-3 sm:px-4 py-3 bg-slate-950/60 gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="ml-1 sm:ml-2 font-mono text-[11px] text-slate-400 truncate max-w-[140px] sm:max-w-none">
                  docketra.in/app/firm/sharma-associates
                </span>
              </div>
                  ...
                  <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-amber-300">Pending Review: DOCKET-2026-MCA-092</span>
                      </div>
                      <span className="rounded bg-amber-500/20 text-amber-300 px-2 py-0.5 text-[10px] font-mono">Stage: Partner Review</span>
                    </div>
                    <p className="text-sm font-semibold text-white mt-2">MGT-7 Annual Return Draft Review for Acme Infotech Pvt Ltd</p>
                    <p className="text-xs text-slate-400 mt-1">Prepared by: Priya Sharma (Associate) • Attached files: Form_MGT7_Final.pdf (Signed)</p>
                    <div className="mt-3 flex items-center gap-2">
                      <span className="rounded bg-emerald-500 px-3 py-1 text-xs font-bold text-slate-950">Approve & Sign-off</span>
                      <span className="rounded border border-red-500/30 bg-red-950/40 px-3 py-1 text-xs font-bold text-red-300">Reject with Notes</span>
                    </div>
                  </div>
                  <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 text-xs text-slate-400">
                    🔒 Immutable Sign-off Audit Trail: Every approval logs timestamp, Partner ID, and digital hash to protect against unvetted submissions.
                  </div>
                </div>
              )}

              {activeSandboxTab === 'importer' && (
                <div className="space-y-3">
                  <div className="rounded-xl border border-slate-800 bg-slate-950 p-4">
                    <div className="flex items-center justify-between text-xs font-bold text-slate-300 mb-2">
                      <span>Dry-Run Preflight Preview: client_master_march2026.xlsx</span>
                      <span className="text-emerald-400 font-mono text-[11px]">42 Valid / 1 Warning</span>
                    </div>
                      ...
                      <table className="w-full text-left text-[11px] font-mono border-collapse">
                        <thead>
                          <tr className="text-slate-400 border-b border-slate-800">
                            ...
                            <th className="py-1 px-2">Entity Name</th>
                            <th className="py-1 px-2">CIN / LLPIN</th>
                            <th className="py-1 px-2">PAN</th>
                            <th className="py-1 px-2">Status</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-800/60">
                          <tr>
                            <td className="py-1 px-2 text-slate-400">1</td>
                            <td className="py-1 px-2 text-white">Aditi Logistics Pvt Ltd</td>
                            <td className="py-1 px-2 text-emerald-400">U60200MH2018PTC304192</td>
                            <td className="py-1 px-2 text-slate-300">AABCA1234F</td>
                            <td className="py-1 px-2 text-emerald-400">Valid</td>
                          </tr>
                          <tr>
                            <td className="py-1 px-2 text-slate-400">2</td>
                            <td className="py-1 px-2 text-white">Horizon Fintech LLP</td>
                            <td className="py-1 px-2 text-emerald-400">AAA-9821</td>
                            <td className="py-1 px-2 text-slate-300">AAFFH5678G</td>
                            <td className="py-1 px-2 text-emerald-400">Valid</td>
                          </tr>
                          <tr>
                            <td className="py-1 px-2 text-slate-400">3</td>
                            <td className="py-1 px-2 text-amber-300">Nova Pharma Ltd</td>
                            <td className="py-1 px-2 text-amber-300">L24230DL1995PLC01234 (20 char)</td>
                            <td className="py-1 px-2 text-slate-300">AABCN9988H</td>
                            <td className="py-1 px-2 text-amber-400">CIN 21-digit check</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                  ...
                  <p className="text-[11px] text-slate-400">
                    💡 Automatic validation catches typo errors before creating records in your live workspace.
                  </p>
                </div>
              )}
            </div>
              ...
              return (
                <div
                  ...
                  className={`rounded-2xl border border-slate-800 transition-all ${
                    isOpen ? 'bg-slate-900 border-amber-500/40 shadow-lg' : 'bg-slate-900/40 hover:border-slate-700'
                  }`}
                >
                  <button
                    type="button"
...
export default DocketraVsExcelWhatsAppPage;
