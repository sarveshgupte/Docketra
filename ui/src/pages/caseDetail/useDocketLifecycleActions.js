  ...
  const handleResolveCase = useCallback(async () => {
    ...
    setConfirmModal({ title:'Resolve Docket', description:`Stage change: ${toLifecycleStage(lifecycleStatus)} → Executed\nTimestamp: ${confirmationTimestamp}\nThis transition will create an audit record.`, confirmText:'Resolve Docket', onConfirm: async () => {
      ...
      try { const response=await caseApi.transitionDocket(caseId,{ toState:'RESOLVED', comment:resolveComment.trim(), sendToQC:forceQcReview, forceQc:forceQcReview });
        if (response.success) { const routedToQc=forceQcReview || ['QC_PENDING','IN_QC'].includes(String(response?.data?.status || response?.data?.state || '').toUpperCase()); const msg=routedToQc?`Docket ${caseId} sent to QC review • ${formatDateTime()}`:`Docket ${caseId} resolved • ${formatDateTime()}`; showSuccess(msg); setActionConfirmation(msg); setActionError(null); setShowResolveModal(false); setResolveComment(''); appendTimelineEvent({ id:`resolved-event-${Date.now()}`, action: routedToQc ? 'QC_PENDING':'RESOLVED', description: resolveComment, createdAt:new Date().toISOString(), createdBy:user?.name||user?.xID||user?.email||'System' }); loadCase({ background:true }); }
