...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  const [
    ...
    Case.countDocuments({ ...baseQuery, status: { $ne: CaseStatus.RESOLVED }, dueDate: { $lt: end } }),
    Case.countDocuments({ firmId: tenantId, createdAt: { $gte: start, $lt: end } }),
    Case.countDocuments({ firmId: tenantId, status: CaseStatus.RESOLVED, resolvedAt: { $gte: start, $lt: end }, createdAt: { $lt: end } }),
    Case.aggregate([
      {
        $match: {
          firmId: tenantId,
