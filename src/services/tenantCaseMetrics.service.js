...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  const matchBase = { firmId: tenantId, createdAt: { $lt: end } };
  ...
  const [
    ...
    Case.countDocuments(matchBase),
    Case.countDocuments({ ...matchBase, status: { $in: [CaseStatus.OPEN, CaseStatus.PENDING, CaseStatus.FILED] } }),
    Case.countDocuments({ ...matchBase, status: CaseStatus.PENDING }),
    Case.countDocuments({ ...matchBase, status: CaseStatus.FILED }),
    Case.countDocuments({ ...matchBase, status: CaseStatus.RESOLVED }),
    Case.countDocuments({ ...matchBase, status: { $in: [CaseStatus.REVIEWED, CaseStatus.UNDER_REVIEW] } }),
    Case.countDocuments({ ...matchBase, dueDate: { $lt: end }, status: { $ne: CaseStatus.RESOLVED } }),
  ...
  return {
    ...
    pendingApprovals,
    overdueCases,
    avgResolutionTimeSeconds: Number(avgRes[0]?.avgResolutionTimeSeconds || 0),
    casesCreatedToday,
    casesResolvedToday,
  };
}
