...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  const [
    ...
    Case.countDocuments(baseMatch),
    Case.countDocuments({ ...baseMatch, status: { $in: [CaseStatus.OPEN, CaseStatus.PENDING, CaseStatus.FILED] } }),
    Case.countDocuments({ ...baseMatch, status: CaseStatus.PENDING }),
    Case.countDocuments({ ...baseMatch, status: CaseStatus.FILED }),
    Case.countDocuments({ ...baseMatch, status: CaseStatus.RESOLVED }),
    Case.countDocuments({ ...baseMatch, status: { $in: [CaseStatus.REVIEWED, CaseStatus.UNDER_REVIEW] } }),
    Case.countDocuments({ ...baseMatch, status: { $ne: CaseStatus.RESOLVED }, dueDate: { $lt: end } }),
    Case.aggregate([
      { $match: { ...baseMatch, status: CaseStatus.RESOLVED, resolvedAt: { $ne: null } } },
      { $group: { _id: null, avgResolutionTimeSeconds: { $avg: { $divide: [{ $subtract: ['$resolvedAt', '$createdAt'] }, 1000] } } } }
    ]),
  ...
  const avgResolutionTimeSeconds = resolvedTotals.length > 0 ? resolvedTotals[0].avgResolutionTimeSeconds : 0;
