...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  // ⚡ Bolt: Revert $facet for performance
  // 💡 What: Replaced memory-intensive $facet aggregation with independent aggregate and countDocuments queries via Promise.all().
  // 🎯 Why: $facet forces MongoDB to pull all matching documents into memory to evaluate sub-pipelines, bypassing indexes and risking the 100MB limit. Independent queries resolve using fast index scans.
  const [
    [totalsResult = {}],
    casesCreatedToday,
    casesResolvedToday,
