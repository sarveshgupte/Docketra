...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  // 💡 What: Replaced memory-intensive $facet aggregation with concurrent aggregation and countDocuments queries via Promise.all().
  ...
  const [totalsArray, casesCreatedToday, casesResolvedToday] = await Promise.all([
    Case.aggregate([
      {
        $match: {
          firmId: tenantId,
