...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  // 💡 What: Replaced memory-intensive $facet aggregation with concurrent aggregation/count queries via Promise.all().
  // 🎯 Why: $facet forces MongoDB to pull all matching documents into memory to evaluate sub-pipelines, bypassing indexes and risking the 100MB memory limit. Individual queries can use indexes efficiently.
