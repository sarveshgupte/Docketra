...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  // 💡 What: Replaced memory-intensive $facet aggregation with concurrent operations via Promise.all().
  // 🎯 Why: $facet forces MongoDB to pull all matching documents into memory, bypassing indexes and risking the 100MB aggregation limit.
  const [[result = {}], casesCreatedToday, casesResolvedToday] = await Promise.all([
    Case.aggregate([
      {
        $match: {
          firmId: tenantId,
