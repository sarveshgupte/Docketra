...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  // 💡 What: Replaced memory-intensive $facet aggregation with concurrent Promise.all() Mongoose queries.
  // 🎯 Why: $facet forces MongoDB to pull all matching documents into memory to evaluate sub-pipelines, bypassing indexes and risking the 100MB memory limit. Individual countDocuments queries can be resolved using fast index scans.
  const [
    totalCases,
    openCases,
    pendedCases,
  ...
  return {
    ...
    avgResolutionTimeSeconds: Number(avgResult.avgResolutionTimeSeconds || 0),
    casesCreatedToday: casesCreatedToday || 0,
    casesResolvedToday: casesResolvedToday || 0,
  };
}
