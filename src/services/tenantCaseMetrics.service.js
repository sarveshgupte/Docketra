...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  const [
    ...
    Case.countDocuments({ firmId: tenantId, createdAt: { $lt: end } }).exec(),
    Case.countDocuments({ firmId: tenantId, createdAt: { $lt: end }, status: { $in: [CaseStatus.OPEN, CaseStatus.PENDING, CaseStatus.FILED] } }).exec(),
    Case.countDocuments({ firmId: tenantId, createdAt: { $lt: end }, status: CaseStatus.PENDING }).exec(),
    Case.countDocuments({ firmId: tenantId, createdAt: { $lt: end }, status: CaseStatus.FILED }).exec(),
    Case.countDocuments({ firmId: tenantId, createdAt: { $lt: end }, status: CaseStatus.RESOLVED }).exec(),
    Case.countDocuments({ firmId: tenantId, createdAt: { $lt: end }, status: { $in: [CaseStatus.REVIEWED, CaseStatus.UNDER_REVIEW] } }).exec(),
    Case.countDocuments({ firmId: tenantId, createdAt: { $lt: end }, dueDate: { $lt: end }, status: { $ne: CaseStatus.RESOLVED } }).exec(),
    Case.countDocuments({ firmId: tenantId, createdAt: { $gte: start, $lt: end } }).exec(),
    Case.countDocuments({ firmId: tenantId, status: CaseStatus.RESOLVED, resolvedAt: { $gte: start, $lt: end } }).exec(),
    Case.aggregate([
      {
        $match: {
          firmId: tenantId,
      ...
      },
      {
        $group: {
          ...
          avgResolutionTimeSeconds: {
            $avg: { $divide: [{ $subtract: ['$resolvedAt', '$createdAt'] }, 1000] }
          }
        }
      }
    ]).exec()
  ...
  const avgResolutionTimeSeconds = avgResult.length > 0 ? avgResult[0].avgResolutionTimeSeconds : 0;
  ...
  return {
    ...
    pendedCases: pendedCases || 0,
    filedCases: filedCases || 0,
    resolvedCases: resolvedCases || 0,
    pendingApprovals: pendingApprovals || 0,
    overdueCases: overdueCases || 0,
    avgResolutionTimeSeconds: avgResolutionTimeSeconds || 0,
    casesCreatedToday: createdToday || 0,
    casesResolvedToday: resolvedToday || 0,
  };
}
