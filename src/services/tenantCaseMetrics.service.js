...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  // 💡 What: Replaced memory-intensive $facet aggregation with concurrent aggregate() and countDocuments() via Promise.all().
  // 🎯 Why: While $facet groups multiple count operations into a single network roundtrip, it forces MongoDB to pull all matching documents into memory. Individual countDocuments queries can be resolved entirely using index scans.
  const [totalsResult, createdTodayCount, resolvedTodayCount] = await Promise.all([
    Case.aggregate([
      {
        $match: {
          firmId: tenantId,
  ...
  const result = {
    totalCases: rawTotals.totalCases || 0,
    openCases: rawTotals.openCases || 0,
    pendedCases: rawTotals.pendedCases || 0,
    filedCases: rawTotals.filedCases || 0,
    resolvedCases: rawTotals.resolvedCases || 0,
    pendingApprovals: rawTotals.pendingApprovals || 0,
    overdueCases: rawTotals.overdueCases || 0,
    avgResolutionTimeSeconds: rawTotals.avgResolutionTimeSeconds || 0,
