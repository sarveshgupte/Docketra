...
async function computeTenantDailyMetrics(tenantId, dateInput) {
  ...
  const [
    ...
    Case.countDocuments({ ...baseMatch, createdAt: { $gte: start, $lt: end } }).exec(),
    Case.countDocuments({ ...baseMatch, status: CaseStatus.RESOLVED, resolvedAt: { $gte: start, $lt: end } }).exec(),
    Case.aggregate([
      {
        $match: {
          ...baseMatch,
