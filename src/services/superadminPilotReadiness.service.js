...
const buildPilotReadinessSnapshot = async () => {
  const [firmHealth, diagnostics, onboarding, planTotals, recentAuditCount, featureFlags] = await Promise.all([
    ...
    // 💡 What: Replaced SuperadminAudit.countDocuments() with SuperadminAudit.exists() since we only need to know if recent audits exist (> 0).
    // 🎯 Why: countDocuments forces a full index scan. exists() provides an O(1) early return upon finding the first match, avoiding unnecessary computation.
    // 📊 Impact: Eliminates an O(N) database query in favor of an O(1) check, reducing database load and speeding up pilot readiness evaluation.
    // 🔬 Measurement: Observe pilot readiness endpoint latency and MongoDB CPU usage.
    SuperadminAudit.exists({ createdAt: { $gte: new Date(Date.now() - (14 * 24 * 60 * 60 * 1000)) } }),
  ...
  const checklist = [
    ...
    { key: CHECKLIST_KEYS[7], label: 'Audit logging readiness', status: recentAuditCount ? 'pass' : 'watch', summary: `Recent superadmin audit metadata events (14d): ${recentAuditCount ? 'Present' : 'None'}.`, evidence: 'Audit metadata presence check only; no payload details returned.', nextAction: recentAuditCount ? 'Continue monitoring audit pipeline.' : 'Verify audit event flow in staging.', href: '/app/superadmin/audit' },
