...
const getComplianceControlRoom = async (firmId, filters = {}) => {
  ...
  // ⚡ Bolt: Removed $facet in favor of concurrent countDocuments().
  // 💡 What: Decomposed $facet aggregation into independent countDocuments queries.
  // 🎯 Why: Replaced memory-intensive $facet with parallel countDocuments that can be resolved using index scans for large datasets.
  const [
    dueThisWeek,
    overdue,
    awaitingClient,
