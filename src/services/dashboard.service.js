...
const getComplianceControlRoom = async (firmId, filters = {}) => {
  ...
  // ⚡ Bolt: Replace memory-intensive $facet with concurrent countDocuments and find queries.
  // 💡 What: Replaced $facet aggregation with concurrent Promise.all queries.
  // 🎯 Why: $facet loads all matched documents into memory, risking the 100MB aggregation memory limit and bypassing index use for sub-pipelines. Separate countDocuments queries leverage fast index scans directly.
  const [
    dueThisWeek,
    overdue,
    awaitingClient,
    ...
    Case.countDocuments({ ...match, compliance_state: COMPLIANCE_STATES.AWAITING_CLIENT }).exec(),
    Case.countDocuments({ ...match, compliance_state: COMPLIANCE_STATES.AWAITING_PARTNER }).exec(),
    Case.countDocuments({ ...match, compliance_state: COMPLIANCE_STATES.READY_TO_FILE }).exec(),
    Case.countDocuments({ ...match, compliance_state: COMPLIANCE_STATES.BLOCKED }).exec(),
    Case.countDocuments({
      ...match,
