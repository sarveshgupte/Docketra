...
const getComplianceControlRoom = async (firmId, filters = {}) => {
  ...
  // 💡 What: Replaced memory-intensive $facet aggregation with concurrent countDocuments() calls.
  ...
  const [dueThisWeek, overdue, awaitingClient, awaitingPartner, readyToFile, blocked, filedRecently, items] = await Promise.all([
    Case.countDocuments({ ...match, compliance_state: { $in: Array.from(ACTIVE_COMPLIANCE_STATES) }, $or: [{ statutory_due_date: { $gte: now, $lte: weekEnd } }, { internal_due_date: { $gte: now, $lte: weekEnd } }] }),
    Case.countDocuments({ ...match, compliance_state: { $in: Array.from(ACTIVE_COMPLIANCE_STATES) }, $or: [{ statutory_due_date: { $lt: now } }, { internal_due_date: { $lt: now } }] }),
    ...
    Case.countDocuments({ ...match, compliance_state: COMPLIANCE_STATES.FILED, filed_at: { $gte: recentFiledStart, $lte: now } }),
    Case.find(match)
  ...
  const summaryPayload = { dueThisWeek, overdue, awaitingClient, awaitingPartner, readyToFile, blocked, filedRecently };
