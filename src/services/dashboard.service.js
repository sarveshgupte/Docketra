...
const getRiskBrief = async (firmId) => {
  ...
  // ⚡ Bolt: Group sequential queries concurrently
  // 💡 What: Merged the sequential `stalePending` countDocuments query into this existing Promise.all array.
  // 🎯 Why: Prevents a redundant network roundtrip by executing the independent stalePending query concurrently with the rest of the risk metrics.
  // 📊 Impact: Reduces database latency for getRiskBrief by ~1 network roundtrip.
  const [
    atRiskEntities,
    waitingClient,
    awaitingApproval,
...
const getApprovalQueues = async (firmId, {
  ...
  // ⚡ Bolt: Group sequential queries concurrently
  // 💡 What: Merged the sequential `items` find query into this existing Promise.all array alongside the count metrics.
  // 🎯 Why: Prevents a redundant network roundtrip. Retrieving the paginated items is independent of the summary counts, allowing MongoDB to execute them concurrently.
  // 📊 Impact: Reduces database latency for getApprovalQueues by ~1 network roundtrip.
