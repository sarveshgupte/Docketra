...
const resendSetup = async (req, res) => {
  ...
  // 💡 What: Replaced unbounded AuthAudit.countDocuments() with bounded find().limit(3)
  // 🎯 Why: countDocuments forces a full index scan. Using find().limit(3) provides O(1) early return performance since we only care if the count reaches 3.
