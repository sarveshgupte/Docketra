...
const resendSetup = async (req, res) => {
  ...
  // ⚡ Bolt Performance Optimization:
  // Replaced unbounded countDocuments() with find().select('_id').limit(3).lean()
  // Impact: Avoids full index scan by returning early after finding the threshold number of documents.
  // Expected improvement: O(1) early return performance for rate limiting checks.
  const recentRecords = await AuthAudit.find({
    userId: user._id,
