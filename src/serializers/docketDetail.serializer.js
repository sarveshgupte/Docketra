...
function serializeDocketDetailDto({ caseObject = {}, client = null, ownerTeam = null, routedTeam = null, assignedUser = null, attachments = [], timeline = [] } = {}) {
  ...
  return {
    ...
    sop: caseObject.sopSnapshot ? {
      ...
      files: Array.isArray(caseObject.sopSnapshot.files) ? caseObject.sopSnapshot.files.map((file, index) => ({
        id: toStringOrNull(file?.id),
        fileName: toStringOrNull(file?.fileName) || '',
        mimeType: toStringOrNull(file?.mimeType) || '',
        size: Number.isFinite(Number(file?.size)) ? Number(file.size) : 0,
        storageProvider: toStringOrNull(file?.storageProvider) || '',
        storageFileId: toStringOrNull(file?.storageFileId),
        objectKey: toStringOrNull(file?.objectKey),
        webViewLink: toStringOrNull(file?.webViewLink),
        uploadedAt: file?.uploadedAt || null,
        uploadedByXID: toStringOrNull(file?.uploadedByXID),
        uploadedByName: toStringOrNull(file?.uploadedByName),
        description: toStringOrNull(file?.description) || '',
