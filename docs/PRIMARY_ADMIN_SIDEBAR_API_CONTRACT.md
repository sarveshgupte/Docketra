...
| Administration | Team & Access | `/app/firm/:firmSlug/admin?tab=users` | `GET /api/admin/stats`, `GET /api/admin/users`, `GET /api/admin/workbaskets`, `GET /api/admin/hierarchy` | `200` with stats default zeros and list payloads as `data: []` in empty state | PRIMARY_ADMIN/ADMIN (stricter split deferred) |
