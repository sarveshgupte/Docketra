...
| Administration | Settings | `/admin?tab=settings` | `GET /api/admin/firm-settings`, `GET /api/admin/cms-intake-settings` | `200` with normalized settings defaults | PRIMARY_ADMIN/ADMIN (read); write/regenerate actions are PRIMARY_ADMIN-only |
| Settings Child | Firm Settings | `/settings/firm` | `GET /api/admin/firm-settings` | `200` with safe defaults object | PRIMARY_ADMIN/ADMIN (read); `PUT /api/admin/firm-settings` is PRIMARY_ADMIN-only |
