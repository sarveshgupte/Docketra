...
Primary Admins can verify the active storage provider and storage map in **Storage Settings → Data storage map**. The section shows provider status, connected account context, canonical location notes, metadata categories, and standardized Google Drive path conventions. Firms can also generate the **Download Data Residency Summary** export for audit workflows.
