...
(async () => {
  ...
  assert.deepStrictEqual(category.subcategories[0].sop.links, []);
  ...
  const payload = { name: 'GST', workbasketId: '507f1f77bcf86cd799439012', deadlineRule: { mode: 'NONE' }, checklistTemplate: [], sop: { title: 'Runbook', body: 'Do A', format: 'markdown', links: [{ title: 'Portal', url: 'https://example.com', type: 'portal' }, { title: 'Ref', url: 'http://example.org', description: 'desc' }] } };
  ...
  assert.throws(() => categoryRouteSchemas['POST /:id/subcategories'].body.parse({ ...payload, sop: { ...payload.sop, links: [{ title: 'bad', url: 'ftp://bad' }] } }));
  ...
  const svc = buildService({ mongoose:{Types:{ObjectId:{isValid:()=>true}}},randomUUID:()=> 'r',createHash:()=>({update(){return this;},digest(){return 'h';}}),
    ...
    categoryRepository:{findActiveCategory: async()=>({name:'GST', subcategories:[{id:'s1', name:'GSTR-1', isActive:true, workbasketId:'wb', deadlineRule:{mode:'NONE', allowManualOverride:true}, checklistTemplate:[{id:'c1', title:'Check'}], sop:{title:'SOP', body:'Steps', format:'markdown', links:[{id:'l1', title:'Portal', url:'https://portal.test', type:'portal', sortOrder:2},{id:'l2', title:'Guide', url:'https://guide.test', description:'Read this', sortOrder:1}]}}]})},
  ...
  assert.strictEqual(captured.sopSnapshot.links.length, 2);
  assert.strictEqual(captured.sopSnapshot.links[1].title, 'Guide');
  ...
  assert.strictEqual(dto.sop.links[0].title, 'Portal');
