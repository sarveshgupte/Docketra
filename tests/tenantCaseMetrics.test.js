...
async function testAggregationIdempotencyAndAccuracy() {
  ...
  Case.countDocuments = async (query) => {
    if (query.createdAt && query.createdAt.$gte) {
      if (query.resolvedAt) return 2; // casesResolvedToday
      return 4; // casesCreatedToday
    }
    if (query.dueDate) return 3; // overdueCases
    if (query.status) {
      if (query.status.$in) {
        if (query.status.$in.includes('OPEN')) return 5; // openCases
        if (query.status.$in.includes('REVIEWED')) return 1; // pendingApprovals
      }
