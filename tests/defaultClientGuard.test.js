...
async function testAuthenticateFallsBackToDatabaseRoleWhenTokenRoleMissingOrStale() {
  let nextCalled = false;
  let verifyCalls = 0;
  const userRecord = {
    _id: 'user-2',
      ...
      return {
        ...
        verifyAccessToken: () => {
          ...
          return verifyCalls === 1
            ? { userId: 'user-2', firmId: 'firm-2' }
            : { userId: 'user-2', firmId: 'firm-2', role: 'USER' };
        },
      };
    }
    if (request === '../utils/role.utils') {
  ...
  for (const expectedRole of ['ADMIN', 'ADMIN']) {
    const req = {
      method: 'GET',
      headers: { authorization: 'Bearer token' },
    ...
    assert.strictEqual(req.user.role, expectedRole, 'request role should fall back to the current DB role');
    assert.strictEqual(req.jwt.role, expectedRole, 'effective JWT role should fall back to the current DB role');
  }

  console.log('  ✓ auth middleware falls back to the database role when token role is missing or stale');
}
...
async function run() {
  try {
    ...
    await testAuthenticateFallsBackToDatabaseRoleWhenTokenRoleMissingOrStale();
