...
## 2024-08-14 - Fix hardcoded IDs in reusable components
**Learning:** Dropdowns and interactive elements using `aria-expanded` require a unique `aria-controls` referencing an element's ID. Using hardcoded IDs can lead to conflicts and screen reader issues when multiple instances exist on the page.
**Action:** Use React's `useId()` to generate unique IDs for ARIA attributes linking `aria-controls` to corresponding elements.
